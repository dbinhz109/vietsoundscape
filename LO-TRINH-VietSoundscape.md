...
| B0.3 | ~~Chốt React hay JS thuần; dựng khung repo, CI, formatter~~ **XONG một phần** — JS thuần đã chốt (`src/app/main.js`, không khung giao diện, Vite chỉ để gói); repo `github.com/dbinhz109/vietsoundscape` (**public từ 21/09**, spec S0.1); CI `.github/workflows/ci.yml` chạy test → validate → build trên mỗi push (spec S0.3); `pages.yml` đưa `main` lên GitHub Pages (A7.1). **Chưa có** formatter/linter, chưa có `LICENSE` (thiếu tên chủ sở hữu) | PM | Q7 trả lời được: clone sạch `npm ci && npm test` xanh 433/433 |
