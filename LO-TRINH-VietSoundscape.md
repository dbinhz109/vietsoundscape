...
| A5.1 | Việc không ảnh hưởng thực nghiệm: **PWA offline + cài được lên màn hình chính**, hiển thị dạng sóng/phổ | PM + ÂT | 📒 21/09: **PWA XONG** — `src/app/offline/` (luật cache có 14 test với cache giả; `sw.js` gói riêng ở gốc bản dựng; `manifest.webmanifest` + biểu tượng 192/512). Kiểm trình duyệt thật: mở một lần rồi tắt mạng, tải lại vẫn đủ 4 địa điểm và phòng nghe 5 lớp từ cache. Kéo lên sớm vì phục vụ C7.2. 📒 21/09 (tối): **dạng sóng XONG** — `src/audio/waveform.js` (đường bao đỉnh, RMS, thứ tự Krause; 15 test) + `src/app/ui/layer-stack.js` (canvas xếp chồng theo nhóm, 13 test). Không phải trang trí: đây là chỗ **thấy** được cấu trúc phân lớp mà H1 nói tới. Chuẩn hoá theo lớp to nhất để vừa nhìn được vừa giữ đúng tương quan độ to; kéo một lớp về 0 thì dải mờ hẳn. **XONG TRỌN A5.1** |
...
| C7.1 | Viết báo cáo/bài báo; **sửa 5 lỗi trong thuyết minh gốc** theo BA §16 (thiếu mục 9, sai lĩnh vực, thiếu kế hoạch/kinh phí/tham khảo, thu hẹp phát biểu "chưa có bản đồ nào", xoá câu H3) | NC | 📒 21/09: **văn bản thay thế soạn sẵn** cho cả 6 lỗi + 3 mục mới ở `nghien-cuu/sua-thuyet-minh.md`. 📒 21/09 (tối): **báo cáo nghiên cứu** `nghien-cuu/bao-cao-nghien-cuu.md` — mục 1 Đặt vấn đề, 2 Cơ sở khoa học, 3 Phương pháp hệ thống, **4 Phương pháp thực nghiệm viết xong** (đăng ký trước nên đã cố định, viết sau khi thấy dữ liệu mới là chỗ dễ kể lại cho khớp kết quả); mục 5–7 có khung kèm danh sách **bắt buộc báo cáo dù kết quả ra sao** và bảng bốn khả năng của hai phép so sánh |
