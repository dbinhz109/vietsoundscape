...
>
> 📒 **21/09 (tối)** — Thêm: **S3.1 máy chấm trước** 33 bản nghe thử (`npm run prescreen`: QUA 20 · CẢNH BÁO 12 · LOẠI 1; thứ tự nghe trong `DUYET.md`; `survey.candidates` ghi vào `clips.json`) — tai người còn ≈ 40 phút thay vì 2,5 giờ; **Q-30** chủ trì cho máy chọn tạm 10 mẫu (`npm run chon:tam`), 5 mẫu đề nghị tự thu — `survey` in riêng, không tính tiến độ; **C2.1/C5.2** quy trình tuyển + thư mời + `npm run participants` (`tuyen-nguoi-tham-gia.md`); **A5.1 PWA** ngoại tuyến + cài được (kiểm tắt mạng tải lại vẫn chạy); **A3.3 khung 12 bản trộn** giữ chỗ (FR-59 hết cảnh báo, 12/12 cân bằng) + nút chọn bản trộn; **A0.3d** hai thư Sở soạn sẵn; **A3.1** khung kế hoạch đợt 2 Huế + Buôn Ê Đê. 632 test.
